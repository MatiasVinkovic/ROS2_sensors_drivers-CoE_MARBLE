...
Protocole reverse-engineered:
  ...
  - Fire message requis pour declencher le stream (0x0015)
...
class OculusReader:
    """Lecteur bufferise de messages Oculus via TCP."""
...
def connect(ip, timeout=15.0, retries=3):
    """Connexion TCP au sonar. Retourne (socket, OculusReader)."""
    for attempt in range(retries):
        try:
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            sock.settimeout(timeout)
            sock.connect((ip, TCP_PORT))
            # Envoyer un fire message pour declencher le stream
            ...
            if attempt < retries - 1:
                import time
                time.sleep(2)
    raise ConnectionError(f"Impossible de se connecter a {ip}:{TCP_PORT}")


def extract_ping(payload):
    """Extrait les metadonnees et l'image d'un ping Oculus.
    Supporte les deux formats: PING (0x0022, ~33k) et PING2 (0x0023, ~122k)."""
    plen = len(payload)
    if plen < 200:
        return None
    ...
    if plen > 50000:
        # Format PING2 (apres fire): fire echo en tete
        try:
